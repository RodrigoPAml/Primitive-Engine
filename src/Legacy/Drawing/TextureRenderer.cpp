...
namespace PrimitiveEngine
{
	namespace Drawing
	{
		void TextureRenderer::Draw(const GPU::TexturePtr texture, const glm::vec2& position, const glm::vec2& size, const float rotation)
		{
			...
			if (cam == nullptr)
				Debug::Logging::LogException("TextureRenderer", "Attempt to draw a texture without a camera");

			if (texture == nullptr)
				Debug::Logging::LogException("TextureRenderer", "Attempt to draw an null texture");

			auto instance = Instance();

			if (instance.shader == nullptr)
				Debug::Logging::LogException("TextureRenderer", "Attempt to draw an texture without a shader");

			if (instance.mesh == nullptr)
				Debug::Logging::LogException("TextureRenderer", "Attempt to draw an texture without a mesh");
		...
		void TextureRenderer::Init()
		{
			...
			Debug::Logging::Log("TextureRenderer", "Initialized");
		}
		...
		void TextureRenderer::Release()
		{
			...
			Debug::Logging::Log("TextureRenderer", "Released");
		}
	}
}
