	...
	namespace GPU
	{
		...
		)
		{
			...
			if(verticeShader.size() > 0)
			{
				...
				Debug::Logging::LogException("Shader", "Vertex shader is empty");
			...
			if(!error.empty())
			{
				...
				Debug::Logging::LogException("Shader", error);
			}
			...
			if (this->id == 0)
				Debug::Logging::LogException("Shader", "Failed to create shader, unknow exception");
		}
		...
		{
			if (this->id == 0)
				Debug::Logging::LogException("Shader", "Cannot use an empty shader");
		...
		{
			if (this->id == 0)
				Debug::Logging::LogException("Shader", "Cannot get uniform on an empty shader");
