	...
	namespace GPU
	{
		Vertex::Vertex(const VertexDescriptor& descriptor)
		{
			if(descriptor.buffers.size() == 0 || descriptor.verticesCount == 0)
				Debug::Logging::LogException("Vertex", "Number of buffer or vertices can't be zero");
			...
			if (this->vao == 0)
				Debug::Logging::LogException("Vertex", "Failed to create Vao");
			...
			{
				if (this->vbos[i] == 0)
					Debug::Logging::LogException("Vertex", "Failed to create Vbo");
			}
			...
			for(int i = 0; i < descriptor.buffers.size(); i++)
			{
				if(descriptor.buffers[i].elements.size() == 0)
					Debug::Logging::LogException("Vertex", "Number of elements of buffer can't be empty");
				...
				{
					if (element.count == 0)
						Debug::Logging::LogException("Vertex", "Element of buffer can't have zero size");
		...
		Vertex::Vertex(const VertexDescriptor& descriptor, const VertexIndicesDescriptor& indicesDescriptor) : Vertex(descriptor)
		{
			if (indicesDescriptor.count == 0 || indicesDescriptor.indices == nullptr)
				Debug::Logging::LogException("Vertex", "Indices can't be empty or null");
			...
			if (this->ibo == 0)
				Debug::Logging::LogException("Vertex", "Failed to create Ibo");
		...
		{
			if (this->vao == 0)
				Debug::Logging::LogException("Vertex", "Can't activate an empty vertex array");
		...
		{
			if (this->vao == 0)
				Debug::Logging::LogException("Vertex", "Can't draw an empty vertex array");
		...
		{
			if (this->vao == 0)
				Debug::Logging::LogException("Vertex", "Can't activate empty vertex array");

			if(buffer < 0 || buffer >= vbosSize)
				Debug::Logging::LogException("Vertex", "Can't modify buffer out of index");
