...
namespace Utils {
	...
	Image::Image(const std::string& path)
	{
		...
		if (!this->data)
			Debug::Logging::LogException("Failed to open texture: " + path, Debug::LogOrigin::Engine);

		if (this->size.x <= 0 || this->size.y <= 0)
			Debug::Logging::LogException("Failed to open texture because size can't be zero or negative: " + path, Debug::LogOrigin::Engine);
