...
namespace GPU {
	...
	)
	{
		...
		if(verticeShader.size() > 0)
		{
			...
			Debug::Logging::LogException("Vertex shader is empty", Debug::LogOrigin::Engine);
		...
		if(!error.empty())
		{
			...
			Debug::Logging::LogException(error, Debug::LogOrigin::Engine);
		}
	...
	{
		if (this->id == 0)
			Debug::Logging::LogException("Cannot use an empty shader", Debug::LogOrigin::Engine);
	...
	{
		if (this->id == 0)
			Debug::Logging::LogException("Cannot get uniform on an empty shader", Debug::LogOrigin::Engine);
