...
namespace GPU {
	...
	Texture::Texture(const TextureDataConfiguration& configuration, Utils::ImagePtr image)
	{
		...
		if (this->id == 0)
			Debug::Logging::LogException("Failed to create shader", Debug::LogOrigin::Engine);
	}
	...
	Texture::Texture(const TextureConfiguration& configuration)
	{
		...
		if (this->id == 0)
			Debug::Logging::LogException("Failed to create shader, unknow exception", Debug::LogOrigin::Engine);
	}
	...
	{
		if(this->id == 0)
			Debug::Logging::LogException("Cannot use a texture if its empty", Debug::LogOrigin::Engine);
	...
	{
		if (this->id == 0)
			Debug::Logging::LogException("Cannot modify texture if its empty", Debug::LogOrigin::Engine);
		...
		if (start.x > this->size.x ||
			...
			start.y < 0)
		{
			Debug::Logging::LogException("Modifying  texture out of index", Debug::LogOrigin::Engine);
		}
