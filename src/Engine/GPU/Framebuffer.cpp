...
namespace GPU {
	...
	Framebuffer::Framebuffer(const FramebufferConfiguration& configuration)
	{
		...
		if(configuration.textureAttachments.empty())
			Debug::Logging::LogException("Attempt to create a framebuffer without texture attachments", Debug::LogOrigin::Engine);
		...
		{
			if (configuration.textureAttachments.empty())
				Debug::Logging::LogException("Failed to complete framebuffer creation", Debug::LogOrigin::Engine);
		}
		...
		if (this->id == 0)
			Debug::Logging::LogException("Failed to create framebuffer", Debug::LogOrigin::Engine);

		if (this->renderBufferId == 0)
			Debug::Logging::LogException("Failed to create render buffer", Debug::LogOrigin::Engine);
	}
	...
	{
		if (this->renderBufferId == 0)
			Debug::Logging::LogException("Can't use an empty framebuffer", Debug::LogOrigin::Engine);
	...
	TexturePtr Framebuffer::GetTextureAttachment(const unsigned int index) const
	{
		if(index < 0 || index > this->texturesAttachments.size())
			Debug::Logging::LogException("Can't access texture attachment out of index", Debug::LogOrigin::Engine);
